...
public class MeteorSpawnDuo : MonoBehaviour
{
    ...
    void Start()
    {
        float temp = 5.0f * (1 - GameManagerDuo.timeSurvived / GameManagerDuo.limitTimer);
    ...
    void Update()
    {
        if (!GameManagerDuo.isMeteorSpawn) this.gameObject.SetActive(false); // 메테오 스폰을 하지 않는다.
    ...
    private void spawnEnemy() 
    {
        ...
        GameObject enemy = PhotonNetwork.Instantiate($"Meteor{rand}Duo", randEnemyPos, Quaternion.identity);
